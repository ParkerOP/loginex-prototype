...
export default function Map({ currentLocation, path }: MapProps) {
  ...
  const center: [number, number] = currentLocation ? [currentLocation.lat, currentLocation.lng] : defaultCenter;

  const polylinePositions: [number, number][] = path.map(p => [p.latitude, p.longitude]);
